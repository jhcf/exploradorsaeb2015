...
names(saeb)[48] <- 'Q041_AÇÕES.REDUÇÃO.TAXA.ABANDONO'
names(saeb)[49] <- 'Q042_AÇÃO.REDUÇÃO.TAXAS.REPROVAÇÃO'
names(saeb)[50] <- 'Q043_AÇÃO.REFORLO.ESCOLAR.APRENDIZ.'
...
ui <- fluidPage(
  ...
  sidebarPanel(
    ...
    sliderInput('tamanhoAmostra', 'Tamanho da amostra', min=1, max=nrow(dataset),
                value=min(1000, nrow(dataset)), step=100, round=0),
  ...
  output$mosaico <- renderPlot({
    ...
    tabelaContingencia <-
      xtabs(as.formula(sprintf("~%s+%s", input$x, input$y)),
            data=saeb[sample(nrow(saeb), input$tamanhoAmostra),])
